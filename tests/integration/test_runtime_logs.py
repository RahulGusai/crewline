...
def _runtime_log_payload(ticket_id: int, agent_id: str = "be", model: str | None = "claude-sonnet-4-6") -> dict:
    return {
        "ticket_id": ticket_id,
        "agent_id": agent_id,
...
async def test_runtime_log_filter_and_classification(
    ...
    response = await pm_client.get(f"/tickets/{ticket['id']}/runtime-logs", params={"agent_id": "be"})
...
async def test_runtime_log_agent_identity_must_match_body(
    ...
    ticket = await create_ticket(owner_agent_id="be")
    payload = _runtime_log_payload(ticket["id"], agent_id="fe")
